...
const parsedStudents = lines.map((line, idx) => {
  ...
  return {
    ...
    role: "student",
    name,
    displayName: isAditya ? "Aditya Shinde" : name.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' '),
    email,
    password: isAditya ? "password123" : null,
    canLogin: isAditya,
    rollNo,
    ...
    parentName: isAditya ? "Santosh Shinde" : `${name.split(' ').slice(1).join(' ') || "Guardian"}`,
    ...
    bloodGroup: isAditya ? "O+" : bloodGroups[idx % bloodGroups.length],
    collegeName: "CSMSS Chh. Shahu College of Engineering",
    cgpa: isAditya ? 7.23 : +(7.0 + ((idx * 13) % 28) / 10).toFixed(2)
  };
});
...
export const TE_VLSI_STUDENTS = ${JSON.stringify(parsedStudents, null, 2)};

export const TE_VLSI_ATTENDANCE = {
${parsedStudents.map((s, idx) => {
  // Generate realistic attendance per subject (80% - 95%)
  const baseAtt = 18 + (idx % 5);
  const total = 24;
  return `  "${s.id}": {
    "sub-vlsi501": { total: 24, attended: ${Math.min(total, baseAtt)}, percentage: ${+((Math.min(total, baseAtt) / total) * 100).toFixed(1)} },
    "sub-vlsi502": { total: 22, attended: ${Math.min(22, baseAtt - 1)}, percentage: ${+((Math.min(22, baseAtt - 1) / 22) * 100).toFixed(1)} },
    "sub-vlsi503": { total: 20, attended: ${Math.min(20, baseAtt)}, percentage: ${+((Math.min(20, baseAtt) / 20) * 100).toFixed(1)} },
    "sub-vlsi504": { total: 22, attended: ${Math.min(22, baseAtt + 1)}, percentage: ${+((Math.min(22, baseAtt + 1) / 22) * 100).toFixed(1)} },
    "sub-vlsi505": { total: 20, attended: ${Math.min(20, baseAtt)}, percentage: ${+((Math.min(20, baseAtt) / 20) * 100).toFixed(1)} },
    "sub-vlsi506": { total: 20, attended: ${Math.min(20, baseAtt - 1)}, percentage: ${+((Math.min(20, baseAtt - 1) / 20) * 100).toFixed(1)} },
    "sub-vlsi507": { total: 16, attended: ${Math.min(16, Math.floor(baseAtt * 0.75))}, percentage: 87.5 },
...
console.log('Wrote TE VLSI students and attendance to', outputFilePath);
